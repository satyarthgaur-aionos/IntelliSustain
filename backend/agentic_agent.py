...
• "Show temperature for Device [DEVICE_ID]"
• "Check humidity for [DEVICE_NAME]"
• "What's the battery level of [DEVICE_NAME]?"
...
• "Show alarms for Device [DEVICE_ID]"
...
This helps me provide the most relevant sustainability insights."""
    ...
    def _handle_error_gracefully(self, error: Exception, user_query: str) -> str:
        ...
        if "openai" in error_str or "rate_limit" in error_str or "429" in error_str:
            return "🤖 I'm experiencing some temporary issues with my AI processing. Please wait a moment and try your request again. If the issue persists, contact support."
        
        # Network/API connectivity errors
        elif "connection" in error_str or "timeout" in error_str or "unreachable" in error_str:
            return "🌐 I'm having trouble connecting to the Inferrix systems right now. Please check your internet connection or try again in a few minutes."
        
        # Authentication errors
        elif "auth" in error_str or "token" in error_str or "unauthorized" in error_str:
            return "🔒 I'm having trouble authenticating with the Inferrix systems. Please log in again or contact your administrator."
        
        # General errors
        else:
            return f"⚠️ I encountered an unexpected issue while processing your request: '{user_query}'. Please try rephrasing or contact support if the problem persists.\nError Details: {str(error)}"
    ...
    def _validate_function_parameters(self, function_name: str, arguments: Dict) -> Optional[str]:
        ...
        if function_name in device_required_functions:
            ...
            if not device_id:
                return "🤔 I need to know which specific device you're referring to. Please provide the device ID or select from the dropdown."
        ...
        if function_name == "hotel_room_comfort_control":
            ...
            if not room_number:
                return "🤔 I need to know which hotel room you'd like me to adjust. Please specify the room number or select from the dropdown."
        ...
        if function_name == "hotel_guest_experience_optimization":
            ...
            if not guest_id:
                return "🤔 I need to know which guest you'd like me to optimize the experience for. Please provide the guest ID or select from the dropdown."
        ...
        if function_name == "acknowledge_alarm":
            ...
            if not alarm_id and not device_id:
                return (
                    "🤔 I need to know which alarm you'd like me to acknowledge. "
                    "Please specify either: the alarm ID (e.g., Acknowledge alarm [ALARM_ID]), "
                    "the device with an alarm (e.g., Acknowledge alarm for Device [DEVICE_ID]), "
                    "or first check alarms (e.g., Show active alarms) then acknowledge a specific one. "
        ...
        if function_name == "comfort_adjustment_control":
            ...
            if not location:
                return "🤔 I need to know which location you'd like me to adjust. Please specify the location or select from the dropdown."
            
            if not adjustment:
                return "🤔 I need to know what type of adjustment you'd like me to make. Please specify the adjustment or select from the dropdown."
        
        # Energy optimization that needs zone specification
        if function_name == "energy_optimization_control":
            zone = arguments.get('zone')
            if not zone:
                return "🤔 I need to know which zone you'd like me to optimize energy for. Please specify the zone or select from the dropdown."
    ...
    def _make_api_request(self, endpoint: str, method: str = "GET", data: Optional[Dict] = None) -> Dict:
        ...
        headers = {"X-Authorization": f"Bearer {get_inferrix_token()}", "Content-Type": "application/json"}
        ...
        try:
            ...
            if isinstance(devices_data, dict):
                ...
                if 'data' in devices_data and 'error' not in devices_data:
                    devices = devices_data['data']
                    if not devices:
                        return "❌ No devices found in the system. Please check your device onboarding or contact support."
                    # Count online/offline devices
                    online_devices = [d for d in devices if d.get('status') == 'online']
                    offline_devices = [d for d in devices if d.get('status') == 'offline']
                    low_battery_devices = [d for d in devices if d.get('battery_level', 4.2) < 3.0]
                    ...
                    if 'how many' in query_lower and 'online' in query_lower:
                        ...
                        if low_battery_devices:
                            ...
                            for d in low_battery_devices[:10]:  # Show first 10
                                summary += f"- {d.get('name', d.get('id', 'Unknown'))} (Battery: {d.get('battery_level', 'Unknown')}V)\n"
    ...
    def _get_device_telemetry(self, args: Dict) -> str:
        try:
            ...
            if not device_id:
                return "❌ Please provide a specific device ID for telemetry data. For example: 'Show temperature for device [DEVICE_ID]'"
            def is_uuid(val):
                return isinstance(val, str) and len(val) == 36 and val.count('-') == 4
            if not is_uuid(device_id):
                mapped_id = self._map_device_name_to_id(device_id)
            ...
            # Determine which telemetry key to retrieve based on query
            ...
            telemetry_data = self._make_api_request(f"plugins/telemetry/DEVICE/{device_id}/values/timeseries?keys={telemetry_key}&startTs={int((datetime.datetime.now() - datetime.timedelta(hours=1)).timestamp() * 1000)}&endTs={int(datetime.datetime.now().timestamp() * 1000)}&limit=10")
            ...
            if isinstance(telemetry_data, dict) and telemetry_data:
                ...
                telemetry_values = telemetry_data.get('data', [])
                if telemetry_values:
                    latest_value = telemetry_values[-1] if telemetry_values else None
                    if latest_value:
                        value = latest_value.get('value', 'Unknown')
                        timestamp = latest_value.get('ts', 'Unknown')
                        return f"📊 **{telemetry_key.title()} for Device {device_id}:**\n• Current Value: {value}\n• Timestamp: {timestamp}\n• Data Points: {len(telemetry_values)}"
                    else:
        ...
        try:
            ...
            if isinstance(device_data, dict):
                ...
                if battery_level != 'Unknown':
                    response += f"• **Battery:** {battery_level}V\n"
    ...
    def _determine_function_from_query(self, query: str) -> Optional[str]:
        ...
        if any(word in query_lower for word in ['temperature', 'humidity', 'battery', 'occupancy']) and any(word in query_lower for word in ['device']):
        ...
        try:
            ...
            if isinstance(devices_data, dict) and 'data' in devices_data:
                ...
                for device in devices:
                    ...
                    # Accept numeric ID match in name (e.g., "123456" in "IAQ Sensor V2 - 123456")
                    if device_name.isdigit() and device_name in name:
                        return device_id
                    
                    # Accept partial name match (e.g., "PIR Sensor Tag - 123456" matches "PIR Sensor Tag - 123456")
